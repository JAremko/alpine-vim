...
RUN apk --update add python git ctags curl && \
...
RUN cd /home/developer/bundle/ && \
    ...
    git clone https://github.com/amix/vimrc/tree/master/sources_non_forked/taglist.vim && \
...
RUN apk --update add --virtual build-deps golang perl bash python-dev build-base \
